...
@onready var target : Array = [offset_left + 5, offset_top + 5, offset_right - 5, offset_bottom - 5]
var margin : Array 
...
func insta_box_size(target : Array) -> void:
	...
	offset_top = target[1] - 5
	offset_right = target[2] + 5
	offset_bottom = target[3] + 5
	resize_finished.emit()

func _process(delta : float) -> void:
	if(auto):
		margin = [offset_left + 5, offset_top + 5, offset_right - 5, offset_bottom - 5]
		...
		match(resize_mode):
			...
			1: 
				...
				if(margin[0] == target[0] && margin[2] == target[2]):
					for j in [1, 3]:
						if abs(margin[j] - target[j]) <= spd: margin[j] = target[j]
						elif margin[j] > target[j]: margin[j] -= spd
						else: margin[j] += spd
			2:
				for i in [1, 3]:
					if abs(margin[i] - target[i]) <= spd: margin[i] = target[i]
				...
				if(margin[1] == target[1] && margin[3] == target[3]):
					for j in [0, 2]:
						if abs(margin[j] - target[j]) <= spd: margin[j] = target[j]
						elif margin[j] > target[j]: margin[j] -= spd
						else: margin[j] += spd
		...
		offset_left = margin[0] - 5
		offset_top = margin[1] - 5
		offset_right = margin[2] + 5
		offset_bottom = margin[3] + 5

	pivot_offset = size / 2.0
	$corner_bottom.position = size - Vector2(5, 5)
	$corner_bottom_inner.position = size - Vector2(10, 10)
	collisions[0].shape.size = Vector2(5,size.y-10)
	collisions[0].position = collisions[0].shape.size / 2
	collisions[1].shape.size = Vector2(size.x-10, 5)
	collisions[1].position = collisions[1].shape.size / 2
	collisions[2].shape.size = Vector2(5,size.y-10)
	collisions[2].position = collisions[2].shape.size / 2 + Vector2(size.x - 15,0)
	collisions[3].shape.size = Vector2(size.x-10, 5)
	collisions[3].position = collisions[3].shape.size / 2 + Vector2(0,size.y - 15)
